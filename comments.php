...
<!-- Add Comment begin -->
<div id="respond">
	<div id="addcomment">发表评论</div>
	<div id="cancel-comment-reply"><?php cancel_comment_reply_link('取消回复') ?></div>
<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
	<p>你必须 <a href="<?php echo get_settings('home'); ?>/wp-login.php">登录后</a> 才能评论！</p>
<?php else : ?>
	<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
<?php if ( $user_ID ) : ?>
	<div class="memberinfo">
    	<a href="/wp-admin/profile.php" target="_blank" title="<?php global $current_user; get_currentuserinfo(); echo $current_user->display_name; ?>,您可以点击头像进入个人中心~"><?php if (function_exists('get_avatar')) { echo get_avatar( get_the_author_email(), '40' ); }?></a>
    <strong><a href="/wp-admin/profile.php" title="点此进入个人中心" target="_blank"><?php echo $user_identity; ?></a></strong> <span>[ 会员 ]</span><br /><?php if (get_option('users_can_register')) { ?><a href="/wp-admin/profile.php" title="个人中心" class="underline">进入个人中心</a><?php }?>  | <a href="<?php echo wp_logout_url(get_permalink()) ?>" title="退出系统" class="underline">退出</a>
    </div>
...
<?php else : ?>
	<p class="welcome_author">
		...
		?> 
            <div class="memberinfo">
                <a href="<?php echo $comment_author_url; ?>" target="_blank" title="<?php echo "$lastCommenter"; ?>"><?php if (function_exists('get_avatar')) { echo get_avatar( $comment_author_email , '44' ); }?></a>
            <strong><a href="<?php echo $comment_author_url; ?>" title="个人网站"><?php echo "$lastCommenter"; ?></a></strong> <span>[您现在是游客身份]</span><br /><?php if (get_option('users_can_register')) { ?>
            <a href="<?php echo get_settings('home'); ?>/wp-login.php?action=register" target="_blank title="注册会员" id="userregister" class="underline">您可以注册会员</a> | <a href="<?php echo get_settings('home'); ?>/wp-login.php" target="_blank title="会员登录" id="userlogin" class="underline">或者登录</a><?php }?>
            </div>
		...
		<?php } else { ?><?php if (get_option('users_can_register')) { ?><p><a href="join-us" class="underline">快速注册</a> |<span id='infotips'>友情提示: 非会员填写以下评论信息, 再次访问参与评论时就不需要重复输入了! </span></p><?php } ?>
            <div id="userform">
                <p><label>用户名：</label><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" /> <em>* </em></p>
                <p><label>电子邮箱：</label><input type="text" name="email" id="email" value="<?php echo $comment_author_email; ?>" size="22" tabindex="2" /> <em>* </em>(保密)</p>
                <p><label>网站网址：</label><input type="text" name="url" id="url" value="<?php echo $comment_author_url; ?>" size="22" tabindex="3" /></p>
...
<?php endif; ?>
        ...
        <p class="input_submit"><input name="submit" type="submit" id="submit" tabindex="5" title="提交评论" value="提交评论" /><?php comment_id_fields(); ?><span class="repeattip">Ctrl+Enter 快捷回复</span></p>
