...
<div class="content3">
    <ul class="explist">
    	<?php if ( get_option('wpyou_exptitlecats_perpage') ) { ?>
	<?php $exptitleperpage = stripslashes(get_option('wpyou_exptitlecats_perpage')); ?>
        <?php } else { ?>
        <?php $exptitleperpage = 10; ?>
        <?php } ?>
        <?php $categoryID=$cat; query_posts('cat='. $categoryID.'orderby=date&order=desc&posts_per_page='.$exptitleperpage.'&paged='.$paged); ?>
	<?php if (have_posts()) : ?>
